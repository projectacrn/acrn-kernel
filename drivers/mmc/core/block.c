				...
				u64 num_of_cmds)
{
	...
	req = blk_get_request(mq->queue, op_mode, __GFP_RECLAIM);
	if (IS_ERR(req)) {
		err = PTR_ERR(req);
		goto cmd_err;
	}
...
static void mmc_blk_rpmb_add(struct mmc_card *card)
{
	...
	list_for_each_entry(rpmb, &md->rpmbs, node) {
		rdev = rpmb_dev_register(&rpmb->dev, &mmc_rpmb_dev_ops);
		if (IS_ERR(rdev)) {
