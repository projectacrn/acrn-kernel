...
struct isi_wl_proc {
	struct hlist_node waiting_wl_proc_node;
	unsigned int async_cmd_req_bitmap;
...
static inline void send_data_wakeup_wl_proc(struct isi_wl_proc *wl_proc,
		void *buf, uint32_t len)
{
	memcpy(wl_proc->rx_buf,	(void *)buf, (len * 4));
	wl_proc->rx_msg_cnt = len * 4;
	hlist_del_init(&wl_proc->waiting_wl_proc_node);
	wake_up_interruptible(&wl_proc->wait);
}
...
static irqreturn_t isi_irq_thread_fn(int irq, void *dev_id)
{
	...
	if (CIRC_CNT(head, tail,
		...
		spin_unlock_irqrestore(&pctx->ring_buf_lock, flags);
		dev_err(pctx->dev,
				"[ISI_IRQ_THRD] no data in circ_buff\n");
		return IRQ_HANDLED;
	}
	do {
		...
		spin_lock(&pctx->proc_waitlist_lock);
		if (req_type >= 0) { /* Async command */
			if (!hlist_empty(&pctx->async_cmd_waitlist)) {
				hlist_for_each_entry(wl_proc, &pctx->async_cmd_waitlist,
						waiting_wl_proc_node) {
					bitmask = (1 << req_type);
					if (wl_proc->async_cmd_req_bitmap &
							bitmask) {
						send_data_wakeup_wl_proc(wl_proc,
								rx_buf, len);
					}
				}
			}
			...
			if (!hlist_empty(&pctx->sync_cmd_waitlist)) {
				hlist_for_each_entry(wl_proc, &pctx->sync_cmd_waitlist,
						waiting_wl_proc_node) {
					if (command == WL_REG_CMD) {
						/* Find waiting proc with seq_num
						 * and wl_id = broadcast_id
						 */
						if ((wl_proc->seq_num == seq_num) &&
		...
		spin_unlock(&pctx->proc_waitlist_lock);
		rx_buf += len;
	} while(1);
	return IRQ_HANDLED;
}
...
static void read_rx_data(struct isi_driver_ctx *pctx)
{
	...
	buf_len = CIRC_SPACE(head,
			tail,
			(SZ_256));
	if (!buf_len)
		goto err_read_rx_data;

	recv_data_len =	ioread32(pctx->mmio + TX_LEN_REG);
	if (!recv_data_len) {
		dev_err(pctx->dev,
				"[ISI]Recv RX interrupt but no RX data\n");
		goto err_read_rx_data;
	}
	if (buf_len < (recv_data_len * 4)) {
		dev_err(pctx->dev,
				"[ISI]Insufficient space for recv_data\n");
		goto err_read_rx_data;
	}

	while(recv_data_len > 0) {
		cb->buf[head] = ioread32(pctx->mmio + PCIE_MB_TX_RAM_OFFSET + (cnt * 4));
...
irqreturn_t isi_pci_dev_irq_handler(int irq, void *ctx)
{
	...
	/* Mask the interrupts */
	intr_mask = BIT(DB_IN_CLR_INTR_MSK_BIT) |
		BIT(DB_OUT_SET_INTR_MSK_BIT);
	...
	if (intr_status & BIT(DB_IN_CLR_INTR_STS_BIT)) {
		...
		pctx->tx_ack = true;
		wake_up_interruptible(&pctx->tx_ack_wait);
		clr_status = 0;
...
static int isi_hw_send(struct isi_driver_ctx *pctx,
		void *tx_buf, int tx_size)
{
	...
	memcpy_toio(pctx->mmio + PCIE_MB_RX_RAM_OFFSET,
			tx_buf, tx_size * 4);
	iowrite32(len, pctx->mmio + RX_LEN_REG);
	tlp_cnt = 1U;
	iowrite32(tlp_cnt, pctx->mmio + TLP_CNT_REG);
	doorbell_in  = (1 << DB_IN_BIT);
	iowrite32(doorbell_in, pctx->mmio + DB_IN_REG);
	
	/*wait for irq_handler to signal tx ack*/
	ret = wait_event_interruptible(pctx->tx_ack_wait,
			(pctx->tx_ack != false));
	pctx->tx_ack = false;
		...
		struct isi_wl_proc *wl_proc, bool is_async)
{
	...
	if (is_async)
		hlist_add_head(&wl_proc->waiting_wl_proc_node,
 ...
 */
static int workload_reg_dereg(unsigned int cmd,
		struct isi_wl_proc *wl_proc,
		struct isi_workload_details *wl_details)
{
	...
	ret = isi_hw_send(pctx, wl_proc->tx_buf, wl_proc->tx_size);
	if (!ret)
		ret = wait_event_interruptible(wl_proc->wait,
	...
	if (wl_proc->rx_msg_cnt > 0) {
		...
		memcpy((void *)&wl_details->resp_header, wl_proc->rx_buf,
				sizeof(struct isi_generic_packet_header));
		dest_ptr = (uint8_t *)(wl_proc->rx_buf) +
			sizeof(struct isi_generic_packet_header);
		rem_size = wl_proc->rx_msg_cnt -
			sizeof(struct isi_generic_packet_header);
		if (copy_to_user(wl_details->resp_buffer,
					dest_ptr, rem_size)) {
			return -EINVAL;
		}
		wl_details->resp_buffer_size =
...
static struct isi_wl_proc *create_wl_proc(struct isi_wl_ctx *p_wl_ctx)
{
	...
	INIT_HLIST_NODE(&p_proc->waiting_wl_proc_node);
	p_proc->p_wl_ctx = p_wl_ctx;
	init_waitqueue_head(&p_proc->wait);
 ...
 */
static int add_async_cmd_requests_to_bitmap(
		uint32_t *requests_arr,
 ...
 */
static int rem_async_cmd_requests_from_bitmap(
		uint32_t *requests_arr,
		unsigned int num_requests,
		unsigned int *bitmap)
{
 ...
 *
 */
static int reg_dereg_async_notify(unsigned int cmd,
		struct isi_wl_proc *wl_proc,
		struct isi_asynch_cmd_request *async_req)
{
	int requests_size = async_req->number_of_requests *
		sizeof(uint32_t);
	unsigned int cmd_req_bitmap = 0;
	int ret = 0;

	if ((2 * SZ_1K) <= requests_size ||
			async_req->requests == NULL)
		return -EINVAL;

	if (copy_from_user(wl_proc->tx_buf,
				async_req->requests,
				requests_size)) {
		return -EINVAL;
	}
	...
	if (cmd == ISI_DRIVER_ASYNCH_CMD_REGISTER) {
		ret = add_async_cmd_requests_to_bitmap(wl_proc->tx_buf,
				async_req->number_of_requests,
				&cmd_req_bitmap);
	} else {
		ret = rem_async_cmd_requests_from_bitmap(wl_proc->tx_buf,
				async_req->number_of_requests,
				&cmd_req_bitmap);
	}
	if (ret)
		...
		struct isi_synch_data_request *sync_req)
{
	...
	memcpy(wl_proc->tx_buf, (void *)&sync_req->req_header,
			sizeof(struct isi_generic_packet_header));
	...
	if ((sync_req->req_buffer != NULL)
		...
		if (copy_from_user((void *)dest_ptr,
					sync_req->req_buffer,
					(sync_req->req_buffer_size * 4))) {
			return -EINVAL;
		}
	}
	if (wl_proc->p_wl_ctx->wl_id !=
			sync_req->workload_id) {
		return -EINVAL;
	}
	...
	ret = isi_hw_send(pctx, wl_proc->tx_buf, wl_proc->tx_size);
	if (!ret && sync_req->is_resp_required) {
		if (timeout_jiffies) {
			ret = wait_event_interruptible_timeout(wl_proc->wait,
					(wl_proc->rx_msg_cnt > 0),
					timeout_jiffies);
			if (ret < 0) {
				proc_waitlist_del_safe(pctx, wl_proc);
	...
	if (wl_proc->rx_msg_cnt > 0) {
		memcpy((void *)&sync_req->resp_header, wl_proc->rx_buf,
				sizeof(struct isi_generic_packet_header));
		dest_ptr = (uint8_t *)(wl_proc->rx_buf) +
			sizeof(struct isi_generic_packet_header);
		rem_size = wl_proc->rx_msg_cnt -
		...
		if (copy_to_user(sync_req->resp_buffer,
					dest_ptr, rem_size)) {
			ret = -EINVAL;
		} else {
		...
		struct isi_asynch_data *async_data)
{
	...
	if (wl_proc->rx_msg_cnt > 0) {
		memcpy((void *)&async_data->header_buffer, wl_proc->rx_buf,
				sizeof(struct isi_generic_packet_header));
		...
		rem_size = wl_proc->rx_msg_cnt -
			sizeof(struct isi_generic_packet_header);
		wl_proc->async_cmd_req_bitmap = 0;
		if (copy_to_user(async_data->data_buffer,
					dest_ptr, rem_size)) {
			ret = -EINVAL;
			goto err_async_notify_get;
		}
		async_data->data_size =
			(rem_size) / sizeof(uint32_t);
	}
...
static long isi_dev_ioctl(struct file *p_file,
 ...
 */
static struct isi_wl_ctx *create_wl_ctx(struct isi_driver_ctx *pctx,
									pid_t tgid)
{
	struct isi_wl_ctx *p_wl_ctx;
		...
		hlist_for_each_entry(pos, &pctx->wl_list, wl_ctx_node) {
			if (pos->tgid == tgid) {
				...
				if (!hlist_empty(&wl_ctxt->wl_proc_list)) {
					hlist_for_each_entry(wl_proc,
							&wl_ctxt->wl_proc_list,
							wl_proc_node) {
						proc_waitlist_del_safe(pctx, wl_proc);
						hlist_del_init(&wl_proc->wl_proc_node);
...
static int isi_dev_ctx_init(struct isi_driver_ctx *pctx)
{
	...
	init_waitqueue_head(&pctx->tx_ack_wait);
...
static int isi_pci_probe(struct pci_dev *pdev,
		const struct pci_device_id *id)
{
	...
	ret = devm_request_threaded_irq(&pctx->pdev->dev,
			pctx->pdev->irq,
			isi_pci_dev_irq_handler,
			isi_irq_thread_fn,
			0, "isi-irq-handler", pctx);
	if (ret)
		return ret;
...
static int __init isi_dev_init(void)
{
	...
	status = alloc_chrdev_region(
			&isi_devt, 0, ISI_NR_DEVICES, DEVICE_NAME);
	if (status)
		return status;
