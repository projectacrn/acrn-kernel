 ...
 * Init functions.
 */
int sw_ipc_mmio_descriptor_init_func_i(struct sw_driver_io_descriptor
				       *descriptor);
 ...
 */
int sw_ipc_mmio_descriptor_reset_func_i(const struct sw_driver_io_descriptor
					*descriptor);
				       ...
				       *descriptor)
{
	...
	if (__ipc_mmio->data_address) {
		__ipc_mmio->data_remapped_address =
		    (pw_u64_t) (unsigned long)ioremap_nocache((unsigned long)
							      __ipc_mmio->
							      data_address,
							      descriptor->
...
void sw_read_ipc_info_i(char *dst_vals, int cpu,
			...
			u16 counter_size_in_bytes)
{
	u16 cmd = descriptors->ipc_descriptor.command, sub_cmd =
	    descriptors->ipc_descriptor.sub_command;
			 ...
			 u16 counter_size_in_bytes)
{
	...
	if (remapped_address) {
		memcpy_fromio(dst_vals, (void *)remapped_address,
			      counter_size_in_bytes);
	}
	pw_pr_debug("Value = %llu\n", *((u64 *) dst_vals));
}
...
void sw_read_pci_info_i(char *dst_vals, int cpu,
			...
			u16 counter_size_in_bytes)
{
	u32 bus = descriptors->pci_descriptor.bus, device =
	    descriptors->pci_descriptor.device;
			 ...
			 u16 counter_size_in_bytes)
{
	...
	if (likely(cpu == RAW_CPU())) {
		...
		u32 l = write_value & 0xffffffff, h =
		    (write_value >> 32) & 0xffffffff;
...
{
	unsigned long remapped_address =
	    (unsigned long)descriptor->mmio_descriptor.data_remapped_address;
	u64 write_value = descriptor->write_value;

	if (remapped_address) {
		memcpy_toio((void *)remapped_address, &write_value,
			    counter_size_in_bytes);
	}
	pw_pr_debug("Value = %llu\n", *((u64 *) dst_vals));
};
					...
					*descriptor)
{
	...
	if (__ipc_mmio->data_remapped_address) {
		pw_pr_debug("unmapping addr 0x%llx\n",
			    __ipc_mmio->data_remapped_address);
		iounmap((volatile void *)(unsigned long)__ipc_mmio->
