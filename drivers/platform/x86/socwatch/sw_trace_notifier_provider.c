...
int sw_register_trace_hrtimer_expire_entry_i(struct sw_trace_notifier_data
					     *node);
int sw_unregister_trace_hrtimer_expire_entry_i(struct sw_trace_notifier_data
					       *node);
 ...
 */
static DEFINE_PER_CPU(bool, sw_is_valid_wakeup_event) = {
true};
...
static struct sw_trace_notifier_data s_trace_collector_lists[] = {
	{SW_TRACE_COLLECTOR_TRACEPOINT, &s_trace_names[SW_TRACE_ID_CPU_IDLE],
	 &sw_register_trace_cpu_idle_i, &sw_unregister_trace_cpu_idle_i, NULL},
	...
	{SW_TRACE_COLLECTOR_TRACEPOINT,
	 ...
	 &sw_register_trace_sched_wakeup_i, &sw_unregister_trace_sched_wakeup_i,
	 NULL},
	...
	{SW_TRACE_COLLECTOR_TRACEPOINT,
	 ...
	 &sw_register_trace_sched_switch_i, &sw_unregister_trace_sched_switch_i,
	 NULL},
};
...
static struct sw_trace_notifier_data s_notifier_collector_lists[] = {
	{SW_TRACE_COLLECTOR_NOTIFIER,
	 &s_notifier_names[SW_NOTIFIER_ID_SUSPEND_NOTIFIER],
	 &sw_register_suspend_notifier_i, &sw_unregister_suspend_notifier_i},
};
...
void sw_handle_irq_wakeup_i(struct sw_collector_data *node, int irq)
{
	int cpu = RAW_CPU();
	sw_driver_msg_t *msg =
	    GET_MSG_SLOT_FOR_CPU(node->msg, cpu, node->per_msg_payload_size);
	...
	preempt_disable();
	{
		...
		list_for_each_entry(curr, &node->list, list) {
			...
			if (cpumask_test_cpu(RAW_CPU(), mask)) {
				...
				unsigned long prev_jiffs =
				    curr->last_update_jiffies, curr_jiffs =
				    jiffies;
				unsigned long delta_msecs =
				    jiffies_to_msecs(curr_jiffs) -
				    jiffies_to_msecs(prev_jiffs);
				/*
				 * Has it been enough time since the last collection point?
				 ...
				 */
				if (CAS64
				    (&curr->last_update_jiffies, prev_jiffs,
				     curr_jiffs) == false) {
					/*
...
DEFINE_PROBE_FUNCTION(PROBE_HRTIMER_PARAMS)
{
	...
	if (unlikely(node == NULL)) {
		node =
		    GET_COLLECTOR_TRACE_NODE(SW_TRACE_ID_HRTIMER_EXPIRE_ENTRY);
		pw_pr_debug("NODE = %p\n", node);
	}
 ...
 * 1. SUSPEND notifier
 */
int sw_probe_suspend_notifier_i(struct notifier_block *block,
				unsigned long state, void *dummy)
{
	static struct sw_trace_notifier_data *node = NULL;
	if (unlikely(node == NULL)) {
		node =
		    GET_COLLECTOR_NOTIFIER_NODE
		    (SW_NOTIFIER_ID_SUSPEND_NOTIFIER);
		pw_pr_debug("NODE = %p\n", node);
	}
	switch (state) {
	...
	default:
		pw_pr_error
		    ("ERROR: unknown state %lu passed to SWA suspend notifier!\n",
		     state);
		break;
	}
	return NOTIFY_DONE;
