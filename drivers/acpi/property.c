...
static bool acpi_is_property_guid(const union acpi_object *guid)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(prp_guid); i++) {
		if (guid_equal((guid_t *)guid->buffer.pointer, &prp_guid[i]))
			return true;
	}
