...
/**
 * struct mei_virtio_cfg - setting passed from virtio backend
 *
...
/**
 * mei_virtio_hbuf_is_empty - checks if write buffer is empty.
 *
 * @dev: the device structure
 *
 * Return: true always
 */
static bool mei_virtio_hbuf_is_empty(struct mei_device *dev)
{
	/*
...
/**
 * mei_virtio_hbuf_max_len - returns size of FE write buffer.
 *
 * @dev: the device structure
 *
...
/**
 * mei_virtio_synchronize_irq - wait for pending IRQ handlers for all virtqueue
 *
 * @dev: the device structure
 */
				    ...
				    const void *data, size_t data_len)
{
	...
	hbuf = virt_addr_valid(hdr) ? hdr :
		kmemdup(hdr, hdr_len, GFP_KERNEL);

	dbuf = virt_addr_valid(data) ? data :
		kmemdup(data, data_len, GFP_KERNEL);
	...
	if (!virtqueue_add_outbuf(hw->out, sg, 2, (void *)dbuf, GFP_KERNEL)) {
		virtqueue_kick(hw->out);
		/*
		 * Block sending is here.
		 * The reason is that the dbuf needs to be handled synchronously
...
static void mei_virtio_intr_handler(struct work_struct *work)
{
	...
	if (in_has_pending)
		mei_virtio_data_in(hw->in);

	mutex_unlock(&dev->device_lock);
}

static void mei_virtio_config_intr(struct virtio_device *vdev)
{
...
static const struct mei_hw_ops mei_virtio_ops = {
	...
	.hbuf_is_ready = mei_virtio_hbuf_is_empty,
...
static struct virtio_driver mei_virtio_driver = {
	...
	.config_changed = mei_virtio_config_intr,
	.driver = {
		.name = KBUILD_MODNAME,
		.owner = THIS_MODULE,
